<template>
    <z-view style="border-width: 8px" slider :progress="progress">
        <span style="color: var(--accent-text-color)">Air conditioning
            <h1>{{ activeScene }}</h1>
            {{ msg }}
        </span>
        <img slot="image" src="ac.jpg" width="100%" height="100%" style="opacity: 0.3">
        <div slot="extension">
            <z-spot v-for="(device, index) in devices" :button="device.name !== 'AC'"
                :knob="(device.name == 'Temperature' || device.name == 'Wind velocity')" v-bind.sync="device.temp"
    ...
    data() {
        return {
            ...
            scenes: [
                ...
                { name: 'Sleep mode', state: 'deactive', icon: 'fas fa-sort' },
                ...
                var id = setInterval(function () {
                    if (vm.progress >= 100) {
                        ...
                        if (scene === 'Regular mode') {
                            vm.devices = [
                                { name: 'Temperature', state: 'on', temp: { qty: 72, unit: 'F', min: 64, max: 90 } },
                                { name: 'Wind velocity', state: 'on', temp: { qty: 2, unit: 'settings', min: 1, max: 3 } },


                            ]
                        } else if (scene === 'Sleep mode') {
                            vm.devices = [
                                { name: 'Temperature', state: 'on', temp: { qty: 77, unit: 'F', min: 64, max: 90 } },
                                { name: 'Wind velocity', state: 'on', temp: { qty: 1, unit: 'settings', min: 1, max: 3 } },


                            ]
                        } else if (scene === 'Powerful mode') {
                            vm.devices = [
                                { name: 'Temperature', state: 'on', temp: { qty: 20, unit: 'F', min: 64, max: 90 } },
                                { name: 'Wind velocity', state: 'on', temp: { qty: 3, unit: 'settings', min: 1, max: 3 } },


                            ]
                        } else if (scene === 'OFF') {
                            vm.devices = [
                                { name: 'Temperature', state: 'off', temp: { qty: 0, unit: 'F', min: 64, max: 90 } },
                                { name: 'Wind velocity', state: 'off', temp: { qty: 0, unit: 'settings', min: 1, max: 3 } },
